...
std::atomic<std::uint64_t> free_total_size;
...
private:
  ...
  void swap_arrays_and_reserve_pos_and_collect(std::unique_lock<std::mutex>& swap_lock, size_t reserved_size, bool do_reserve = true)
  {
    ...
    {
      LOG() << "waiting for m_used_size == reserved_size -> " << m_used_size.load() << " == " << reserved_size;
    }
  ...
  {
    LOG() << "malloc info array size=" << size;

    std::scoped_lock<std::mutex> allocations_map_lock(allocations_map_mutex);
    ...
    for (std::size_t pos = 0; pos < size; ++pos)
    {
      ...
      auto i = allocations_map.find(malloc_info.ptr);

      if (i == allocations_map.end())
      {
        continue;
      }
